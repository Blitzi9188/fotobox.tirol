...
import { fetchGoogleLatestReviews } from "@/lib/google-reviews";
...
export default async function KontaktPage({
  ...
  const googleReviews = await fetchGoogleLatestReviews(10);
  ...
  const reviews = {
    ...reviewsDefaults,
    ...(content.reviews || {}),
    score: googleReviews?.score || content.reviews?.score || reviewsDefaults.score,
    reviewCountLabel:
      googleReviews?.reviewCountLabel || content.reviews?.reviewCountLabel || reviewsDefaults.reviewCountLabel,
    ctaHref: googleReviews?.ctaHref || content.reviews?.ctaHref || reviewsDefaults.ctaHref,
    items:
      googleReviews?.items && googleReviews.items.length > 0
        ? googleReviews.items
